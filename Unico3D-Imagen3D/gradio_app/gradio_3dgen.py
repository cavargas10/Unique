...
def generate3dv2(preview_img, input_processing, seed, render_video=True, do_refine=True, expansion_weight=0.1, init_type="std"):
    ...
    logging.info(f"[{job_id}] Nueva generación iniciada. Directorio temporal creado en: {output_dir}")
    ...
    try:
        ...
        logging.info(f"[{job_id}] Ejecutando predicción multivista...")
        rgb_pils, front_pil = run_mv(preview_img, input_processing, seed)

        logging.info(f"[{job_id}] Reconstruyendo geometría (Parte 1)...")
        ...
        logging.info(f"[{job_id}] Reconstruyendo geometría (Parte 2)...")
        meshes = geo_reconstruct_part2(vertices, faces)

        logging.info(f"[{job_id}] Reconstruyendo geometría (Parte 3)...")
        ...
        logging.info(f"[{job_id}] Generación completada. Archivos resultantes: {ret_mesh}, {video}")
        return ret_mesh, video

    except Exception as e:
        logging.error(f"[{job_id}] Ha ocurrido un error durante la generación: {e}", exc_info=True)
